...
function shouldTrySameOriginFallback() {
  return API_BASE_URL !== '/api' && typeof window !== 'undefined';
}

async function fetchAdmin(path: string, init: RequestInit = {}) {
  try {
    return await fetch(adminUrl(path), init);
  } catch (error) {
    if (!shouldTrySameOriginFallback()) throw error;
    return fetch(adminUrl(path, '/api'), init);
  }
}
